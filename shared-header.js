...
function updateMarqueeRainbow() {
     ...
     marqueeRainbowLinks.forEach(function (link) {
          ...
          letters.forEach(function (letter, index) {
               letter.style.color = marqueeRainbowPalette[(index + cycleOffset) % marqueeRainbowPalette.length];
          });
     });
}
...
function startMarqueeRainbow() {
     ...
     marqueeRainbowTimer = window.setInterval(updateMarqueeRainbow, marqueeRainbowCycleMs);
}
...
function getSiteTitleTextWidth(siteTitleMarquee) {
     ...
     return Math.max(siteTitleMarquee.scrollWidth, siteTitleMarquee.getBoundingClientRect().width);
}
...
function fitSiteTitleToHeader(header) {
     ...
     const maxTitleWidth = getSiteTitleWidthAtSize(parts.siteTitleMarquee, maxFontSize);
     ...
     const scaledFontSize = Math.floor(maxFontSize * (availableWidth / maxTitleWidth));
