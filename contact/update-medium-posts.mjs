...
async function updateMediumPosts() {
  ...
  // Separate the RSS feed into individual article records before cleaning them.
  ...
  const posts = itemBlocks
    ...
    .map((item) => {
      ...
      const rawContent = getRawTagValue(item, "content:encoded");
      const content = truncate(
        cleanText(rawContent) || getTagValue(item, "description"),
        excerptLength
      );
      const imageUrl = getFirstImageUrl(rawContent);
